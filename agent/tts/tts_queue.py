...
class TtsQueue:
    ...
    async def discard_pending(self) -> None:
        """Drop unsynth sentences for this turn (e.g. replace robotic LLM reply)."""
        self._merge_buf = ""
        while True:
            try:
    ...
    async def enqueue(self, text: str) -> None:
        if self._interrupted.is_set():
            return
        if self._sentences_queued >= self._max_sentences:
            return
    ...
    async def flush(self) -> None:
        ...
        while rest and self._sentences_queued < self._max_sentences:
